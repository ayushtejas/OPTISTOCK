...
const Insight = ({ insightName }) => {
    ...
    const Descriptions = {
        ...
        'Sales': 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed nonne merninisti licere mihi ista probare, quae sunt a te dicta? Huius, Lyco, oratione locuples, rebus ipsis ielunior. Quae cum essent dicta, discessimus.',
        'Production Forecast': 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed nonne merninisti licere mihi ista probare, quae sunt a te dicta? Huius, Lyco, oratione locuples, rebus ipsis ielunior. Quae cum essent dicta, discessimus.'
    }

    return (
        <Box sx={{ height: '100vh', width: '100vw', position: 'relative' }} p='4'>
            <Heading textAlign={'center'} mt={4} size='xl'>{Headers[insightName]}</Heading>
            ...
            <Box sx={{ position: 'absolute', bottom: '0', left: '50%', transform: 'translateX(-50%)', width: '100%' }}>
                <HStack px={'4'} pb={2} spacing={4} h={'150px'} mx={4}>
...
export const getServerSideProps: GetServerSideProps<any> = async ({ params }) => {
    ...
    return {
        props: {
            insightName: insightName || null // Use null as fallback
        }
    };
};
